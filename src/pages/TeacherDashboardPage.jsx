...
import {
  ...
  getCallRowKey,
  getCallStudentName,
  getCallWeeklyStatusLabel,
  getCallWeeklyStatusValue,
} from '../utils/callData';
...
function getTimeBadgeParts(timeValue) {
  ...
  const parts = safeTime
    .split(/\s*[-–—]\s*/)
    .map((part) => formatDisplayTime(part))
    .filter(Boolean);

  return {
    start: parts[0] || 'Saat yok',
    end: parts[1] || '',
  };
}
...
function formatLastUpdated(updatedAt) {
  ...
  const safeDate = updatedAt instanceof Date ? updatedAt : new Date(updatedAt);
...
function getNotificationBannerText(messagingState) {
  if (!messagingState.supported) {
    return 'Bu cihazda web push bildirimleri kullanılamıyor.';
  }

  if (messagingState.permission === 'denied') {
    return 'Bildirim izni tarayıcı ayarlarında kapalı. Açtıktan sonra sayfayı yenileyin.';
  }

  if (messagingState.status === 'loading') {
    return 'Bildirim kurulumu hazırlanıyor...';
  }

  return 'Yeni duyurular için web bildirimlerini açın.';
}
...
function TeacherDashboardPage({
  ...
  const showNotificationBanner = !messagingState.enabled;
  const canPromptForNotifications =
    messagingState.supported
    && messagingState.permission !== 'denied';
  const classesLastUpdatedText = formatLastUpdated(classesState.updatedAt);
  const callsLastUpdatedText = formatLastUpdated(callsState.updatedAt);
  const announcementsLastUpdatedText = formatLastUpdated(announcementsState.updatedAt);
  ...
  return (
    <main className="page-shell page-shell--dashboard">
      <header className="teacher-header">
        <div className="teacher-header__brand">
          <div className="teacher-header__logo-frame">
            <div className="teacher-header__logo-shell">
              <img alt="Aren Academy logosu" className="academy-logo academy-logo--header" src={logoSrc} />
            </div>
          </div>
      ...
      {showNotificationBanner ? (
        <section className={`notification-banner ${messagingState.error ? 'notification-banner--error' : ''}`}>
          <div className="notification-banner__copy">
            <span className="section-kicker">Bildirimler</span>
            <p>{getNotificationBannerText(messagingState)}</p>
          </div>

          {canPromptForNotifications ? (
            <div className="notification-banner__actions">
              <button
                className="button button-primary notification-banner__button"
                disabled={messagingState.status === 'loading'}
                onClick={onEnableNotifications}
                type="button"
              >
                {messagingState.status === 'loading' ? 'Hazırlanıyor...' : 'Bildirimleri aç'}
              </button>

              {messagingState.error ? (
                <p className="notification-banner__error-text">{messagingState.error}</p>
              ) : null}
            </div>
          ) : null}
        </section>
        ...
        <section className="content-card content-card--classes">
          <div className="content-card__header">
            <div>
              ...
              {classesLastUpdatedText ? (
                <p className="content-card__last-updated">{classesLastUpdatedText}</p>
              ) : null}
            </div>
                  ...
                  <div className="class-list">
                    {orderedSelectedClasses.map((item, index) => {
                      const isCompleted = selectedDay
                        ...
                        : false;
                      const timeBadge = getTimeBadgeParts(item.time);
                      ...
                      return (
                        ...
                        >
                          <div className="class-row-item__time-badge">
                            <span className="class-row-item__time-start">{timeBadge.start}</span>
                            {timeBadge.end ? <span className="class-row-item__time-end">{timeBadge.end}</span> : null}
                          </div>
        ...
        <section className="content-card content-card--content-only">
          <div className="content-card__header">
            <div>
              ...
              <h2>Arama listesi</h2>
              {callsLastUpdatedText ? (
                <p className="content-card__last-updated">{callsLastUpdatedText}</p>
              ) : null}
              <p className="content-card__teacher-debug">
                Current teacher used for saving: {teacherNameForSaving || 'No teacher selected'}
              </p>
              <div className="content-card__supabase-debug">
              ...
              {debugWriteState?.message || debugWriteState?.error ? (
                <p className={`content-card__debug-status content-card__debug-status--${debugWriteState.status}`}>
                  {debugWriteState.error || debugWriteState.message}
                </p>
              ) : null}
            </div>
            <div className="content-card__actions">
              {isGokhanTeacher ? (
                <button
                  ...
                  disabled={debugWriteState?.status === 'loading'}
                  onClick={onTestGokhanWrite}
                  type="button"
                >
                  Test GÖKHAN Write
                </button>
            ...
            >
              <div className="call-list">
                {callsState.data.map((item, index) => {
                  ...
                  const telHref = getTelHref(phoneNumber);
                  ...
                  const weeklyStatusValue = getCallWeeklyStatusValue(item);
                  const weeklyStatusLabel = getCallWeeklyStatusLabel(item);
                  const rowSaveState = callSaveState.targetKey === rowKey
                    ? callSaveState
                    : { status: 'idle', error: '' };

                  return (
                    <div className={`call-list__item ${isEditorOpen ? 'call-list__item--expanded' : ''}`} key={rowKey}>
                      <article
                      ...
                      >
                        <div className="call-row__content">
                          ...
                          <span className="call-row__phone">{phoneNumber || 'Telefon numarası yok'}</span>
                          <span className={`call-row__status call-row__status--${weeklyStatusValue}`}>
                            {weeklyStatusLabel}
                          </span>
                        </div>
                        ...
                        <div className="call-row__actions">
                          ...
                          <IconButtonLink
                            ...
                            onClick={async (event) => {
                              ...
                              await onCallAction(item, rowKey, 'call', telHref);
                            }}
                          >
                            <CallActionIcon />
                          </IconButtonLink>
                          <IconButtonLink
                            ...
                            onClick={async (event) => {
                              event.preventDefault();
                              event.stopPropagation();
                              await onCallAction(item, rowKey, 'whatsapp', whatsAppHref);
                            }}
                          >
                            <WhatsAppIcon />
                          </IconButtonLink>
                      ...
                      <CallLogModal
                        noteValue={getCallNoteValue(item)}
                        onNoteChange={(nextNote) => onCallNoteChange(item, rowKey, nextNote)}
                        onNoteSave={(nextNote) => onCallNoteSave(item, rowKey, nextNote)}
        ...
        <section className="content-card content-card--content-only">
          <div className="content-card__header">
            <div>
              ...
              <h2>Son duyurular</h2>
              {announcementsLastUpdatedText ? (
                <p className="content-card__last-updated">{announcementsLastUpdatedText}</p>
              ) : null}
            </div>
